...
- **Admin credentials**: The dev database is seeded with `admin@heroesrise.org` / `admin123`. The password hash is generated via `npm run hash:admin -- "admin123"`.
...
- `npm run hash:admin -- "<password>"` — Generate bcrypt hash for admin user
